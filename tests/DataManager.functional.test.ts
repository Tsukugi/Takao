...
describe('DataManager', () => {
  beforeEach(() => {
    ...
    const actionsData = {
      actions: {
        low_health: [
          { type: 'search', description: '{{unitName}} searches for health.', effect: 'health + 10' }
        ],
        healthy: [
          { type: 'explore', description: '{{unitName}} explores.', effect: 'experience + 5' }
        ],
        default: [
          { type: 'patrol', description: '{{unitName}} patrols.', effect: 'awareness + 5' },
          { type: 'rest', description: '{{unitName}} rests.', effect: 'health + 5' }
        ]
      }
    };
    ...
    const diaryData = [
      { turn: 1, action: { type: 'patrol', player: 'TestWarrior' }, timestamp: new Date().toISOString() },
      { turn: 2, action: { type: 'rest', player: 'TestWarrior' }, timestamp: new Date().toISOString() }
    ];
  ...
  it('loads actions from JSON file correctly', () => {
    ...
    const originalActionsFile = (DataManager as any).ACTIONS_FILE;
    (DataManager as any).ACTIONS_FILE = ACTIONS_FILE;
    ...
    (DataManager as any).ACTIONS_FILE = originalActionsFile;
  });

  it('loads names from JSON file correctly', () => {
    // Temporarily change data paths for testing
    const originalNamesFile = (DataManager as any).NAMES_FILE;
  ...
  it('loads units from JSON file correctly', () => {
    // Temporarily change data paths for testing
    const originalUnitsFile = (DataManager as any).UNITS_FILE;
    (DataManager as any).UNITS_FILE = UNITS_FILE;
    ...
    // Restore original path
    (DataManager as any).UNITS_FILE = originalUnitsFile;
  });

  it('loads diary entries from JSON file correctly', () => {
    // Temporarily change data paths for testing
    const originalDiaryFile = (DataManager as any).DIARY_FILE;
    (DataManager as any).DIARY_FILE = DIARY_FILE;
    ...
    // Restore original path
    (DataManager as any).DIARY_FILE = originalDiaryFile;
  });
  ...
  it('returns 0 when diary file does not exist', () => {
    ...
    const originalDiaryFile = (DataManager as any).DIARY_FILE;
    (DataManager as any).DIARY_FILE = fakeDiaryFile;
