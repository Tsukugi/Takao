...
export class StatTracker {
   ...
   */
  public static takeSnapshot(units: any[]): { [unitId: string]: PropertySnapshot } {
  ...
  public static compareSnapshots(
    ...
    for (const unit of units) {
      ...
      for (const propName of allPropNames) {
        ...
        if (initialPropInfo && typeof initialPropInfo === 'object' && 'value' in initialPropInfo) {
          initialValue = (initialPropInfo as any).value;
        } else {
          initialValue = initialPropInfo;
        }
        
        // Extract value from current property info (could be object with value field or raw value)
        if (currentPropInfo && typeof currentPropInfo === 'object' && 'value' in currentPropInfo) {
          newValue = (currentPropInfo as any).value;
        } else {
          newValue = currentPropInfo;
        }
        
        // Only log if values are defined and different
        if (initialValue !== undefined && newValue !== undefined && initialValue !== newValue) {
          changes.push({
            unitId: unit.id,
            unitName: unit.name,
  ...
  public static formatStatChanges(changes: StatChange[]): string[] {
    ...
    for (const change of changes) {
      formattedChanges.push(`${change.propertyName}: ${change.oldValue} -> ${change.newValue}`);
    }
   ...
   */
  public static groupChangesByUnit(changes: StatChange[]): Map<string, StatChange[]> {
